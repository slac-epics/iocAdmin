/***************************************************************************/
/* Filename: devPTM4200CL.c                                             */
/* Description: EPICS device support for PULNiX PTM4200CL 12B camera       */
/***************************************************************************/
...
int PTM4200CL_DEV_DEBUG = 1;

/* some constants about PULNiX PTM4200CL 12B camera */

#define CAMERA_MODEL_NAME "PTM4200CL"
#define CAMERA_CONFIG_NAME "ptm4200cl_pw.cfg"
...
typedef struct IMAGE_BUF
{
    ...
    unsigned short int	*pImage;	/* PTM4200CL is 12-bit camera */
...
unsigned int PTM4200_SHIFT_4BITS = 0;

static int PTM4200CL_Poll(PTM4200CL_CAMERA * pCamera)
{
    int loop, saveImage;
...
static int PTM4200CL_Start(PTM4200CL_CAMERA * pCamera)
{
    if(!pCamera)
    {
        errlogPrintf("PTM4200CL_Start is called with pCamera=NULL!\n");
        return -1;
    }

    /* In this application, we use PTM4200CL 12B async pulse width mode */
    pdv_enable_external_trigger(pCamera->pCameraHandle, PDV_PHOTO_TRIGGER);
    pdv_start_images(pCamera->pCameraHandle, 0);

    /* Create thread */
#ifdef vxWorks
    taskSpawn(pCamera->pCameraName, CAMERA_THREAD_PRIORITY, VX_FP_TASK, CAMERA_THREAD_STACK, (FUNCPTR)PTM4200CL_Poll, (int)pCamera, 0, 0, 0, 0, 0, 0, 0, 0, 0);
#else
    epicsThreadMustCreate(pCamera->pCameraName, CAMERA_THREAD_PRIORITY, CAMERA_THREAD_STACK, (EPICSTHREADFUNC)PTM4200CL_Poll, (void *)pCamera);
#endif

    return 0;
}

int PTM4200CL_Init(char * name, int unit, int channel)
{
    int status, loop, looprow;

    PTM4200CL_CAMERA * pCamera = (PTM4200CL_CAMERA *) callocMustSucceed( 1, sizeof(PTM4200CL_CAMERA), "Allocate memory for PTM4200CL_CAMERA" );
    /* no bzero needed */

    /* common camera initialization */
    status = commonCameraInit(name, unit, channel, CAMERA_MODEL_NAME, CAMERA_CONFIG_NAME, (CAMERASTARTFUNC)PTM4200CL_Start, (CAMERA_ID)pCamera);
    ...
    if(PTM4200CL_DEV_DEBUG) printf("calloc %fMB memory\n", NUM_OF_FRAMES * pCamera->imageSize/1.0e6);
...
} PTM4200CL_DEV_SUP_SET;

PTM4200CL_DEV_SUP_SET devAiEDTCL_PTM4200_12B=   {6, NULL, NULL, init_ai,  NULL, read_ai,  NULL};
PTM4200CL_DEV_SUP_SET devBoEDTCL_PTM4200_12B=   {6, NULL, NULL, init_bo,  NULL, write_bo,  NULL};
PTM4200CL_DEV_SUP_SET devLiEDTCL_PTM4200_12B=   {6, NULL, NULL, init_li,  NULL, read_li,  NULL};
...
epicsExportAddress(dset, devLoEDTCL_PTM4200_12B);
epicsExportAddress(dset, devWfEDTCL_PTM4200_12B);
#endif
...
typedef enum {
    PTM4200CL_AI_CurFwhmX,
    PTM4200CL_AI_CurFwhmY,
    PTM4200CL_AI_CurCtrdX,
    PTM4200CL_AI_CurCtrdY,
    PTM4200CL_AI_HisFwhmX,
    PTM4200CL_AI_HisFwhmY,
    PTM4200CL_AI_HisCtrdX,
    PTM4200CL_AI_HisCtrdY,
    PTM4200CL_BO_SaveImage,
    PTM4200CL_LI_NumOfCol,
    PTM4200CL_LI_NumOfRow,
    PTM4200CL_LI_NumOfBits,
    PTM4200CL_LI_FrameRate,
    PTM4200CL_LI_StartPixelX,
    PTM4200CL_LI_StartPixelY,
    PTM4200CL_LI_NumPixelsX,
    PTM4200CL_LI_NumPixelsY,
    PTM4200CL_LO_HisIndex,
    PTM4200CL_LO_StartPixelX,
    PTM4200CL_LO_StartPixelY,
    PTM4200CL_LO_NumPixelsX,
    PTM4200CL_LO_NumPixelsY,
    PTM4200CL_WF_CurImage,
    PTM4200CL_WF_HisImage,
    PTM4200CL_WF_CurProjX,
    PTM4200CL_WF_CurProjY,
    PTM4200CL_WF_HisProjX,
    PTM4200CL_WF_HisProjY,
} E_PTM4200CL_FUNC;
...
static struct PARAM_MAP
{
        char param[40];
        E_EPICS_RTYPE rtype;;
        E_PTM4200CL_FUNC funcflag;
} param_map[] = {
    {"CurFwhmX",	EPICS_RTYPE_AI,	PTM4200CL_AI_CurFwhmX},
    {"CurFwhmY",	EPICS_RTYPE_AI,	PTM4200CL_AI_CurFwhmY},
    {"CurCtrdX",	EPICS_RTYPE_AI,	PTM4200CL_AI_CurCtrdX},
    {"CurCtrdY",	EPICS_RTYPE_AI,	PTM4200CL_AI_CurCtrdY},
    {"HisFwhmX",	EPICS_RTYPE_AI,	PTM4200CL_AI_HisFwhmX},
    {"HisFwhmY",	EPICS_RTYPE_AI,	PTM4200CL_AI_HisFwhmY},
    {"HisCtrdX",	EPICS_RTYPE_AI,	PTM4200CL_AI_HisCtrdX},
    {"HisCtrdY",	EPICS_RTYPE_AI,	PTM4200CL_AI_HisCtrdY},
    {"SaveImage",	EPICS_RTYPE_BO,	PTM4200CL_BO_SaveImage},
    {"NumOfCol",	EPICS_RTYPE_LI,	PTM4200CL_LI_NumOfCol},
    {"NumOfRow",	EPICS_RTYPE_LI,	PTM4200CL_LI_NumOfRow},
    {"NumOfBits",	EPICS_RTYPE_LI,	PTM4200CL_LI_NumOfBits},
    {"FrameRate",	EPICS_RTYPE_LI,	PTM4200CL_LI_FrameRate},
    {"StartPixelX",	EPICS_RTYPE_LI,	PTM4200CL_LI_StartPixelX},
    {"StartPixelY",	EPICS_RTYPE_LI,	PTM4200CL_LI_StartPixelY},
    {"NumPixelsX",	EPICS_RTYPE_LI,	PTM4200CL_LI_NumPixelsX},
    {"NumPixelsY",	EPICS_RTYPE_LI,	PTM4200CL_LI_NumPixelsY},
    {"HisIndex",	EPICS_RTYPE_LO,	PTM4200CL_LO_HisIndex},
    {"StartPixelX",	EPICS_RTYPE_LO,	PTM4200CL_LO_StartPixelX},
    {"StartPixelY",	EPICS_RTYPE_LO,	PTM4200CL_LO_StartPixelY},
    {"NumPixelsX",	EPICS_RTYPE_LO,	PTM4200CL_LO_NumPixelsX},
    {"NumPixelsY",	EPICS_RTYPE_LO,	PTM4200CL_LO_NumPixelsY},
    {"CurImage",	EPICS_RTYPE_WF,	PTM4200CL_WF_CurImage},
    {"HisImage",	EPICS_RTYPE_WF,	PTM4200CL_WF_HisImage},
    {"CurProjX",	EPICS_RTYPE_WF,	PTM4200CL_WF_CurProjX},
    {"CurProjY",	EPICS_RTYPE_WF,	PTM4200CL_WF_CurProjY},
    {"HisProjX",	EPICS_RTYPE_WF,	PTM4200CL_WF_HisProjX},
    {"HisProjY",	EPICS_RTYPE_WF,	PTM4200CL_WF_HisProjY}
};
#define N_PARAM_MAP (sizeof(param_map)/sizeof(struct PARAM_MAP))

typedef struct PTM4200CL_DEVDATA 
{
    PTM4200CL_CAMERA * pCamera;
    E_PTM4200CL_FUNC   function;
    dbCommon * pRecord;
    void * pArg;
} PTM4200CL_DEVDATA;

/* This function will be called by all device support */
/* The memory for PTM4200CL_DEVDATA will be malloced inside */
static int PTM4200CL_DevData_Init(dbCommon * precord, E_EPICS_RTYPE rtype, char * ioString)
{
    PTM4200CL_DEVDATA *   pdevdata;

    PTM4200CL_CAMERA * pCamera;

    char    devname[40];
    char    param[40];
    E_PTM4200CL_FUNC    funcflag = 0;
    ...
    pCamera = (PTM4200CL_CAMERA *)devCameraFindByName(devname, CAMERA_MODEL_NAME);
    if(pCamera == NULL)
    {
    ...
    pdevdata = (PTM4200CL_DEVDATA *)callocMustSucceed(1, sizeof(PTM4200CL_DEVDATA), "allocate memory for PTM4200CL_DEVDATA");
...
static long init_ai( struct aiRecord * pai)
{
    ...
    {
        recGblRecordError(S_db_badField, (void *)pai, "devAiEDTCL_PTM4200_12B Init_record, Illegal INP");
        pai->pact=TRUE;
        return (S_db_badField);
    }

    if( PTM4200CL_DevData_Init((dbCommon *)pai, EPICS_RTYPE_AI, pai->inp.value.instio.string) != 0 )
    {
...
static long read_ai(struct aiRecord * pai)
{
    ...
    pdevdata = (PTM4200CL_DEVDATA *)(pai->dpvt);
    pCamera = pdevdata->pCamera;

    switch(pdevdata->function)
    {
    case PTM4200CL_AI_CurFwhmX:
    case PTM4200CL_AI_CurFwhmY:
    case PTM4200CL_AI_CurCtrdX:
    case PTM4200CL_AI_CurCtrdY:
    case PTM4200CL_AI_HisFwhmX:
    case PTM4200CL_AI_HisFwhmY:
    case PTM4200CL_AI_HisCtrdX:
    case PTM4200CL_AI_HisCtrdY:
        /* Calculate projiection, FWHM, centroid ... */
    default:
        return -1;
    }
...
static long init_bo( struct boRecord * pbo)
{
    ...
    {
        recGblRecordError(S_db_badField, (void *)pbo, "devBoEDTCL_PTM4200_12B Init_record, Illegal OUT");
        pbo->pact=TRUE;
        return (S_db_badField);
    }
    ...
    if( PTM4200CL_DevData_Init((dbCommon *)pbo, EPICS_RTYPE_BO, pbo->out.value.instio.string) != 0 )
    {
        errlogPrintf("Fail to init devdata for record %s!\n", pbo->name);
    ...
    pdevdata = (PTM4200CL_DEVDATA *)(pbo->dpvt);
    pCamera = pdevdata->pCamera;

    switch(pdevdata->function)
    {
    case PTM4200CL_BO_SaveImage:
...
static long init_li( struct longinRecord * pli)
{
    ...
    {
        recGblRecordError(S_db_badField, (void *)pli, "devLiEDTCL_PTM4200_12B Init_record, Illegal INP");
        pli->pact=TRUE;
        return (S_db_badField);
    }

    if( PTM4200CL_DevData_Init((dbCommon *)pli, EPICS_RTYPE_LI, pli->inp.value.instio.string) != 0 )
    {
        errlogPrintf("Fail to init devdata for record %s!\n", pli->name);
...
static long read_li(struct longinRecord * pli)
{
    ...
    pdevdata = (PTM4200CL_DEVDATA *)(pli->dpvt);
    pCamera = pdevdata->pCamera;

    switch(pdevdata->function)
    {
    case PTM4200CL_LI_NumOfCol:
        pli->val = pCamera->numOfCol;
        break;
    case PTM4200CL_LI_NumOfRow:
        pli->val = pCamera->numOfRow;
        break;
    case PTM4200CL_LI_NumOfBits:
        pli->val = pCamera->numOfBits;
        break;
    case PTM4200CL_LI_FrameRate:
        pli->val = pCamera->frameCounts;
        pCamera->frameCounts = 0;
        break;
    case PTM4200CL_LI_StartPixelX:
        pli->val = pCamera->startPixelX;
        break;
    case PTM4200CL_LI_StartPixelY:
        pli->val = pCamera->startPixelY;
        break;
    case PTM4200CL_LI_NumPixelsX:
        pli->val = pCamera->nPixelsX;
        break;
    case PTM4200CL_LI_NumPixelsY:
...
static long init_lo( struct longoutRecord * plo)
{
    ...
    {
        recGblRecordError(S_db_badField, (void *)plo, "devLoEDTCL_PTM4200_12B Init_record, Illegal OUT");
        plo->pact=TRUE;
        return (S_db_badField);
    }

    if( PTM4200CL_DevData_Init((dbCommon *)plo, EPICS_RTYPE_LO, plo->out.value.instio.string) != 0 )
    {
        errlogPrintf("Fail to init devdata for record %s!\n", plo->name);
    ...
    pdevdata = (PTM4200CL_DEVDATA *)(plo->dpvt);
    pCamera = pdevdata->pCamera;

    switch(pdevdata->function)
    {
    case PTM4200CL_LO_HisIndex:
...
static long init_wf(struct waveformRecord *pwf)
{
    ...
    {
        recGblRecordError(S_db_badField, (void *)pwf, "devWfEDTCL_PTM4200_12B Init_record, Illegal INP");
        pwf->pact=TRUE;
        return (S_db_badField);
    }

    if( PTM4200CL_DevData_Init((dbCommon *)pwf, EPICS_RTYPE_WF, pwf->inp.value.instio.string) != 0 )
    {
        errlogPrintf("Fail to init devdata for record %s!\n", pwf->name);
...
static long read_wf(struct waveformRecord * pwf)
{
    ...
    pdevdata = (PTM4200CL_DEVDATA *)(pwf->dpvt);
    pCamera = pdevdata->pCamera;
    ...
    switch(pdevdata->function)
    {
    case PTM4200CL_WF_CurImage:
        if(pwf->ftvl != DBF_USHORT || pwf->nelm != pCamera->imageSize/sizeof(unsigned short int))
        {
            recGblRecordError(S_db_badField, (void *)pwf, "devWfEDTCL_PTM4200_12B read_record, Illegal FTVL or NELM field");
            pwf->pact=TRUE;
            return (S_db_badField);
        }
        if(pCurImageBuf)
