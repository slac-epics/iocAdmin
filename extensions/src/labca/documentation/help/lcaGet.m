...
%   returned as character strings; otherwise, all values are converted
%   into double precision numbers. Explicit type conversion into strings
%   can be enforced by submitting the `type' argument described below.
%
%  Parameters
%
%   pvs
...
%          retrieve (i.e. limit the number of columns of value to nmax).
%          If set to 0 (default), all elements are fetched and the number
%          of columns, n, in the result matrix is set to the maximum
%          number of elements among the PVs. The option is useful to limit
%          the transfer time of large waveforms (unfortunately, CA does
%          not return the valid elements (``NORD'') of an array only -- it
%          always ships all elements).
%
%   type
%          (optional argument) A string specifying the data type to be
%          used for the channel access data transfer. Note that unless the
%          PVs are of native ``string'' type or conversion to ``string''
%          is enforced explicitely (type = char), labCA always converts
%          the data to ``double'' locally.
%
...
%          for using anything other than native, the default, which
%          directs CA to use the same type for transfer as the data are
%          stored on the server.
%
%          Occasionally, conversion to char can be useful: retrieve a
%          number of PVs as strings, i.e. let the CA server convert them
%          to strings (if the PVs are not native strings already) and
%          transfer them.
%
%          If multiple PVs are requested, either none or all must be of
%          native DBF_STRING or DBF_ENUM type unless explicit conversion
%          to char is enforced by specifying this argument.
%
...
%          refusal to read PVs with INVALID severity can be tuned using
%          the lcaSetSeverityWarnLevel call as well.
%
%   timestamp
...
%          nanoseconds (imaginary part) elapsed since 00:00:00 UTC, Jan.
%          1, 1970.
%
%  Examples
%
