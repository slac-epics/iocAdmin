...
%          PVs. If there is only a single row in value it is written to
%          all m PVs. value may be a matrix of ``double'' precision
%          numbers or a (matlab: cell-) matrix of strings (in which case
%          the values are transferred as strings and converted by the CA
%          server to the native type -- this is particularly useful for
...
%          used for the channel access data transfer. Note that labCA
%          always converts numerical data from ``double'' locally.
%
%          It can be desirable, to use a different data type for the
%          transfer because by default CA transfers are limited to ~ 16kB.
%          Legal values for type are byte, short, long, float, double,
%          char or native. There should rarely be a need for using
%          anything other than native, the default, which directs CA to
%          use the same type for transfer as the data are stored on the
%          server. If value is a string matrix, type is automatically set
%          to char.
%
