...
/* JROCK SLAC - flag to indicate more verbose cmlog messages */
int _wordyCMLOG_flag = 0;
#endif
...
/* JROCK SLAC - verbose CMLOG messages */
#define PARM_WORDY                              25
struct parm_data
{
	char* parm;
	int len;
...
static struct parm_data ptable[] = {
		...
		{ "-oCM", 4,			PARM_OPMOD_LOG_CMLOG },
                /* JROCK SLAC option for wordy CMLOG messages */
		{ "-wCM", 4,			PARM_WORDY },
#endif
		...
		for(j=0;!finished && !parm_error && ptable[j].parm;j++)
		{
			if(strncmp(ptable[j].parm,argv[i],Mmax(ptable[j].len,strlen(argv[i])))==0)
			{
				...
				case PARM_OPMOD_LOG_CMLOG:
					...
					break;
				case PARM_WORDY:
                                        /* JROCK SLAC - wordy cmlog message option */
					_wordyCMLOG_flag = 1;
					finished=1;
					break;
#endif
...
static void printUsage(char *pgm)
{
	...
	fprintf(stderr,"  -version         Print version number\n");
        /* JROCK SLAC wordy cmlog messages */
	fprintf(stderr,"  -w               Wordy (verbose) CMLOG messages\n");
	exit(1);
}
...
void masterTesting()
{
	...
	else 
	  {
	    ...
	    if(DEBUG) fprintf(stderr,"I'm master;lockFileDeskriptor=%d\n",lockFileDeskriptor);
            /* make sure we have a top level window first */
