...
#ifdef CMLOG
    /* JROCK SLAC added more stuff to message if wordy option selected */
    if (_wordyCMLOG_flag)
       {
       /* get host name and clip at the first . character */
       sprintf(tot_host_name, "%s", ca_host_name(cdata->chid));
       host_name = strtok(tot_host_name, ".");
       if (cdata->alias)
          sprintf(alias_str, "%s%s%s", " (" , cdata->alias, ") ");
       else
          sprintf(alias_str, "%s", " ");

       if (_global_flag) {
            sprintf(text, "%s%schanged to %s, %s, %s/%s",
                    cdata->name,
                    alias_str,
                    cdata->value,
            ...
            sprintf(text, "%s%schanged to %s, %s",
                    cdata->name,
                    alias_str,
                    cdata->value,
                    alhAlarmSeverityString[cdata->curSevr]);
       ...
       cmlog_logmsg(cmlog,
                    ...
                    "text=%s domain=%s value=%s host=%s",
...
void alLogOpModMessage(int messageCode,GCLINK* gclink,const char* fmt,...)
{
    ...
    if (use_CMLOG_opmod) {
      ...
      if (!gcdata) return;
/* JROCK SLAC changed gdata->alias : "N/A" to gcdata->alias: "N/A" - typo? */
/* JROCK SLAC changed cm_text, below (gcdata->alias, to text, - typo?*/
...
#ifdef CMLOG
    if (use_CMLOG_opmod) {
    /* JROCK SLAC changed gcdata->pname, to gcdata->name, - typo? */
    /* JROCK SLAC changed cm_text, below (gcdata->alias to text, - typo? */
